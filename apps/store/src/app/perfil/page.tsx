...
export default async function Perfil() {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center px-4 py-8">
      <section className="bg-card flex flex-col items-center gap-2 px-8 py-6 rounded shadow w-full">
        <div className="flex flex-col items-center justify-center gap-2 py-8 w-full">
          <figure className="flex justify-center items-center p-1 rounded-full border-primary border-2">
            {session.user.profile_image ? (
              ...
              <i className="icon-[solar--user-bold] text-primary w-[96px] h-[96px]"></i>
            )}
          </figure>
          <div className="flex flex-col items-center gap-1 py-1 text-center w-full">
            <h2 className="text-xl font-bold">{session.user.name}</h2>
            <span className="flex justify-center items-start self-stretch gap-1 w-full">
              <i className="icon-[solar--letter-bold] min-w-[16px] min-h-[16px]"></i>
